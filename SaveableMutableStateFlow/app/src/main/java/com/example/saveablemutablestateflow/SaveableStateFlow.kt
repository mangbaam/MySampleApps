package com.example.saveablemutablestateflow
 ...
 *
 * `adb shell am kill "com.example.saveablemutablestateflow"` 명령으로 테스트 가능
 *
...
class SaveableMutableStateFlow<T>(
        ...
        if (!validateValue(initialValue)) {
            if (BuildConfig.DEBUG_MODE) {
                ...
                Log.d(
                    ...
                    "SavedState 에 저장할 수 없는 타입입니다. https://developer.android.com/topic/libraries/architecture/viewmodel-savedstate?hl=ko#direct 에서 지원하는 타입을 확인하세요",
                )
            }
        }
    }
    ...
    fun validateValue(value: T) = SavedStateHandle.validateValue(value)
}
